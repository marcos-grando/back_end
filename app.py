...
# app = Flask(__name__, template_folder='../front_end', static_folder='../front_end')                                            # versão simplificada 
app = Flask(__name__, template_folder='../front_end/versão-completa/templates', static_folder='../front_end/versão-completa')   # projeto completo

current_dir = os.path.dirname(os.path.abspath(__file__))
# db_path = os.path.join(current_dir, 'database/database.db')          # versão simplificada
db_path = os.path.join(current_dir, 'database/database-projeto.db')   # projeto completo
...
# ======================================================
# APIs para carregar dados na versão front-end simplificada

@app.route('/carrega_dados_conv')
def carregar_dados_conv():
    try:
        with sqlite3.connect(db_path, timeout=10) as conn:
            cursor = conn.cursor()

            cursor.execute("SELECT id, nome, designacao, mesa FROM convidados")
            ...
            dados = {
                'lista_convidados': [
                    {'id': row[0], 'nome': row[1], 'designacao': row[2], 'mesa': row[3]}
                    for row in lista_convidados
                ],
                'total_convites': total_convites,
                'total_mesas': total_mesas,
                'user_mesas': [
                    {'mesa': row[0], 'in_mesa': row[1]}
                    for row in user_mesas
                ]
            }
...
@app.route('/carrega_dados_func')
def carregar_dados_func():
    try:
        with sqlite3.connect(db_path, timeout=10) as conn:
            cursor = conn.cursor()

            cursor.execute("SELECT id, nome, funcao FROM organizadores")
            lista_organizadores = cursor.fetchall()

            cursor.execute("SELECT COUNT(*) FROM organizadores")
            total_organizadores = cursor.fetchone()[0]

            cursor.execute("SELECT funcao, COUNT(*) AS total FROM organizadores GROUP BY funcao")
            total_por_funcao = cursor.fetchall()

            dados = {
                'lista_organizadores': [
                    {'id': row[0], 'nome': row[1], 'funcao': row[2]}
                    for row in lista_organizadores
                ],
                'total_organizadores': total_organizadores,
                'total_por_funcao': [
                    {'funcao': row[0], 'total': row[1]}
                    for row in total_por_funcao
                ]
            }
            return jsonify(dados)
